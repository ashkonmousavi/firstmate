#!/usr/bin/env bash
# Live validation driver, round 2 (asserts the accepted seeded-tab residual bound).
...
layout 00-initial
echo "target HEAD: $(git -C "$ROOT" rev-parse HEAD)"
sidebar_only_tab() {  # <tab-id>: every pane in the tab matches the cleanup sidebar predicate
  lab api snapshot | jq -e --arg t "$1" '[.result.snapshot.panes[] | select(.tab_id == $t)] | length > 0 and all(.[]; .label == "Sidebar" and (.agent // null) == null and ((.agent_status // "unknown") == "unknown"))' >/dev/null
}
seeded_count() { lab api snapshot | jq '[.result.snapshot.tabs[] | select(.label == "1")] | length'; }
seeded_ok() {  # at most one tab "1" in the session, and it holds only Sidebar panes
  local n t; n=$(seeded_count); [ "$n" -le 1 ] || return 1
  [ "$n" -eq 0 ] && return 0
  t=$(lab api snapshot | jq -r '.result.snapshot.tabs[] | select(.label == "1") | .tab_id'); sidebar_only_tab "$t"
}
no_task_tabs() { ! lab api snapshot | jq -e 'any(.result.snapshot.tabs[]; .label | startswith("fm-"))' >/dev/null; }
...
check "real plugin Sidebar pane matches the cleanup predicate" \
...
check "residual bound after spawn fa: at most one seeded tab \"1\", holding only Sidebar panes (count=$(seeded_count), tabs=$(tab_labels "$FLAT_WS"))" 'seeded_ok'

FB=$(spawn "$HOME_DIR" fb "$PROJ") || exit 1
FB_TAB=$(tab_of "$FB"); wait_sidebar "$FB_TAB" || echo "NOTE: no plugin Sidebar docked in fb's tab"
layout 02-after-spawn-fb
teardown "$HOME_DIR" fa || exit 1; sleep 1
...
check "projected spawn pj1: has a presentation journal and its own workspace" '[ -f "$HOME_DIR/state/pj1.herdr-presentation" ] && [ "$PJ_WS" != "$FLAT_WS" ]'
teardown "$HOME_DIR" pj1 || exit 1; sleep 1
...
teardown "$HOME_DIR" fb || exit 1; sleep 1
layout 06-after-teardown-fb
check "flat teardown fb (last task): fm-fb tab closed" '! tab_present "$FB_TAB"'
check "flat teardown fb: workspace either removed or left holding only the single sidebar-only seeded tab (tabs=$(tab_labels "$FLAT_WS" 2>/dev/null))" \
  '! ws_present "$FLAT_WS" || { lab tab list --workspace "$FLAT_WS" | jq -e "[.result.tabs[].label] == [\"1\"]" >/dev/null && seeded_ok; }'
check "no fm- task tab lingers after fb" 'no_task_tabs'

# --- Adversarial: repeated spawn/teardown cycles must not accumulate seeded tabs or task tabs.
for c in 1 2 3; do
  id="fc$c"
  FC=$(spawn "$HOME_DIR" "$id" "$PROJ") || exit 1
  FC_WS=$(ws_of "$FC"); FC_TAB=$(tab_of "$FC")
  wait_sidebar "$FC_TAB" || echo "NOTE: no plugin Sidebar docked in $id's tab"
  sleep 2
  layout "07-cycle$c-after-spawn"
  if ws_present "$FLAT_WS"; then
    check "cycle $c spawn $id: reuses the surviving per-home workspace ($FC_WS == $FLAT_WS)" '[ "$FC_WS" = "$FLAT_WS" ]'
  else
    echo "cycle $c: per-home workspace had been removed; $id created $FC_WS"; FLAT_WS=$FC_WS
  fi
  check "cycle $c spawn $id: residual bound holds (seeded count=$(seeded_count), tabs=$(tab_labels "$FC_WS"))" 'seeded_ok'
  teardown "$HOME_DIR" "$id" || exit 1; sleep 1
  layout "08-cycle$c-after-teardown"
  check "cycle $c teardown $id: task tab closed and no fm- tab lingers" '! tab_present "$FC_TAB" && no_task_tabs'
  check "cycle $c teardown $id: residual bound holds (seeded count=$(seeded_count))" 'seeded_ok'
done
check "session-wide: total tabs after 3 cycles <= 1 (tabs=$(lab api snapshot | jq -c '[.result.snapshot.tabs[].label]'))" \
  '[ "$(lab api snapshot | jq ".result.snapshot.tabs | length")" -le 1 ]'
