#!/usr/bin/env bash
# Live driver (terminal-launched variant):  fm-supervise-daemon.sh whose watcher cannot start because a
...
FM_HEARTBEAT=999999 FM_CHECK_INTERVAL=999999 FM_STALE_ESCALATE_SECS=999999 \
  timeout 12 "$ROOT/bin/fm-supervise-daemon.sh" > "$H/daemon.out" 2>&1
rc=$?
