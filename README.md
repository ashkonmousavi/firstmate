...
Codex and OpenCode are also verified and supported as primary harnesses; Codex uses a persistent Stop-owned successor after one bootstrap checkpoint, and OpenCode uses a TUI plugin, so both carry more harness-specific supervision tradeoffs than the three co-primaries.
