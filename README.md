...
| `/updatefirstmate` | Install the latest Firstmate bytes, restart every eligible live second mate on them, and report honestly that the primary loads them only in a fresh session; retain a re-read nudge where a secondmate restart cannot be proven |
