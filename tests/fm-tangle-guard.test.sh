...
#   - durable Treehouse allocation completes before terminal creation, while
#     the worktree wait loop targets that stable window id, never the
#     (possibly-renamed) name - a lost name would let
...
test_spawn_tmux_window_construction() {
  ...
  # Bug 2 fix (b): durable Treehouse allocation is never typed into the pane,
  # and the worktree wait loop still targets the stable id.
  assert_no_grep "send-keys -t @spawnwid treehouse get" "$rec" \
    "durable treehouse get must complete before the worker terminal is created"
