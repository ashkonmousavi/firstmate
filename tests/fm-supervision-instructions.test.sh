...
test_selected_harness_block_only() {
  ...
  assert_contains "$out" "Mode: Codex persistent Stop successor with one bounded bootstrap checkpoint." "codex snippet missing"
  assert_contains "$out" "bin/fm-watch-checkpoint.sh" "codex checkpoint helper missing"
  assert_contains "$out" "do not start another foreground checkpoint" "codex handoff instruction missing"
...
test_repair_lines() {
  ...
  assert_contains "$out" "persistent supervision is Stop-owned" "codex repair line lost successor ownership"
  assert_contains "$out" "retry Stop from Herdr or tmux" "codex repair line lost the supported retry route"
  assert_not_contains "$out" "bin/fm-watch-checkpoint.sh" "codex repair line recreated the checkpoint loop"
  ...
  assert_contains "$out" "retry Stop from Herdr or tmux" "x-mode codex repair line lost the persistent handoff route"
  assert_not_contains "$out" "bin/fm-watch-checkpoint.sh" "x-mode codex repair line recreated the checkpoint loop"
...
test_cross_harness_ordinary_continuation_and_repair_matrix() {
  ...
  out=$("$RENDER" --harness codex)
  ordinary=$(printf '%s\n' "$out" | grep -F -- '- Ordinary wake:')
  assert_contains "$ordinary" "persistent Stop-owned successor" "codex ordinary-wake line lost successor ownership"
  assert_contains "$ordinary" "do not start another foreground checkpoint" "codex ordinary-wake line recreated the checkpoint loop"
  assert_not_contains "$ordinary" "bin/fm-watch-checkpoint.sh" "codex ordinary-wake line retained the checkpoint command"
  ...
  assert_contains "$out" "persistent supervision is Stop-owned" "codex recovery line lost successor ownership"
  assert_contains "$out" "retry Stop from Herdr or tmux" "codex recovery line lost the supported retry route"
  assert_not_contains "$out" "bin/fm-watch-checkpoint.sh" "codex recovery line retained the checkpoint command"
