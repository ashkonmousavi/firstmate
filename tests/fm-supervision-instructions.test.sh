...
test_quiet_mode_stanzas() {
  ...
  mkdir -p "$home/state" "$config"
  out=$(FM_HOME="$home" FM_CONFIG_OVERRIDE="$config" "$RENDER" --harness claude --afk 1 --afk-mode quiet)
  ...
  out=$(FM_HOME="$home" "$RENDER" --harness claude --afk 1 --afk-mode quiet --repair-line)
  assert_contains "$out" "Quiet mode owns watcher supervision; load /quiet" "quiet repair line did not name /quiet"

  out=$(FM_HOME="$home" "$RENDER" --harness claude --afk 1)
  ...
  out=$(FM_HOME="$home" "$RENDER" --harness claude --afk 1 --afk-mode not-a-real-mode)
  assert_contains "$out" "- Away mode: active" "unrecognized --afk-mode value did not fall back to away"

  out=$(FM_HOME="$home" "$RENDER" --harness claude --afk 0)
