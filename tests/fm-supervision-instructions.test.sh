...
test_selected_harness_block_only() {
  ...
  assert_contains "$out" "Mode: Codex persistent turn-end supervision" "codex snippet missing"
  assert_contains "$out" "one bounded bootstrap checkpoint" "codex bounded bootstrap missing"
...
test_repair_lines() {
  ...
  assert_contains "$out" "tracked Codex Stop hook" "codex repair line did not prefer persistent handoff recovery"
  assert_contains "$out" "only if that hook reports the successor unavailable" "codex repair line did not gate diagnostic checkpoint use"
  ...
  assert_contains "$out" "identity-matched persistent successor" "x-mode codex repair line lost persistent handoff ownership"
  assert_contains "$out" "bin/fm-watch-checkpoint.sh --seconds 7" "x-mode codex repair line lost the diagnostic checkpoint"
...
test_cross_harness_ordinary_continuation_and_repair_matrix() {
  ...
  out=$("$RENDER" --harness codex)
  ordinary=$(printf '%s\n' "$out" | grep -F -- '- Ordinary wake:')
  assert_contains "$ordinary" "persistent successor already owns watcher continuity" "codex ordinary-wake line lost persistent ownership"
  assert_contains "$ordinary" "do not start another foreground checkpoint" "codex ordinary-wake line permits a repeated checkpoint"
  assert_not_contains "$ordinary" "bin/fm-watch-checkpoint.sh" "codex ordinary-wake line still starts a checkpoint"
  ...
  assert_contains "$out" "tracked Codex Stop hook" "codex recovery line lost persistent handoff repair"
  assert_contains "$out" "successor unavailable" "codex recovery line did not bound diagnostic fallback"
  assert_contains "$out" "one diagnostic foreground checkpoint" "codex recovery line lost its bounded diagnostic fallback"
