...
# fm_test_prep_record <data-dir> <id> [<q1>] [<q2>]
# Writes an answered record for <id> under <data-dir>. The tier header answers
# default to no/no - tier 0, the cheapest record a ship spawn accepts - so a
# fixture that only needs the gate satisfied pays nothing for it; pass yes to
# either question to exercise a higher tier, where every section is answered.
# Idempotent: an existing record is left alone so a test can write its own.
# Returns non-zero if the scaffold fails.
fm_test_prep_record() {
  local data=$1 id=$2 q1=${3:-no} q2=${4:-no} root prep
