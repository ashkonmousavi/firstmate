...
# These regressions retain upstream's tasks-axi boundary while the merged lint
# owner keeps the bounded one-root execution from the customized home.
...
SH
  ...
  pass "fm-lint.sh rejects representative direct Beads CLI invocations in firstmate core"
}
...
test_rejects_direct_beads_cli_in_explicit_core_path() {
  ...
  pass "fm-lint.sh enforces backend purity for explicit canonical core paths"
}
...
# A launcher may already have made fm-lint.sh the leader of its own session and
# process group. Repeating setpgrp(0, 0) is EPERM on Linux in that state, but
# the requested isolation is already true and lint must continue. The ordinary
# worker created by this run is not a group leader at entry, so this also proves
# the repeated worker entry still performs successful isolation setup.
test_accepts_an_already_owned_process_group() {
  local tmp fakebin fixture out rc
  tmp=$(fm_test_tmproot fm-lint-already-isolated)
  fakebin=$(fm_fakebin "$tmp")
  fixture="$tmp/good.sh"

  command -v perl >/dev/null 2>&1 || {
    pass "SKIP (perl not available): already-owned process-group check"
    return
  }
...
printf 'ALREADY_ISOLATED_SHELLCHECK_DONE\n'
SH
  chmod +x "$fakebin/shellcheck"

  rc=0
  out=$(perl -MPOSIX=setsid -e '
    setsid() >= 0 or die "setsid: $!";
    exec @ARGV or die "exec: $!";
  ' -- env PATH="$fakebin:$PATH" FM_LINT_JOBS=1 bash "$LINT" "$fixture" 2>&1) || rc=$?

  [ "$rc" -eq 0 ] \
    || fail "an already-isolated fm-lint.sh entry failed with exit $rc"$'\n'"$out"
  assert_contains "$out" "ALREADY_ISOLATED_SHELLCHECK_DONE" \
    "an already-isolated fm-lint.sh entry stopped before its real verdict"
  pass "fm-lint.sh accepts an already-owned process group and still completes its worker"
}
