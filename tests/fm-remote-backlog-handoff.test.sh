...
# Tear down deterministically. worker.pid names the serving child beneath its
# restart supervisor, so use the production owner to stop that verified process
# group before draining this shell's own background stages and removing the tree.
