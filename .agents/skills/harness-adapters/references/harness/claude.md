...
Stop payload `stop_hook_active=true` follows any hook-driven continuation, including async reawakening, so Claude mode ignores it and uses cooperative claim and epoch plus bounded re-block; Codex's separate persistent-successor mode also refuses to treat it as supervision proof.
