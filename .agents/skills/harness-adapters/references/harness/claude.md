...
A crewmate launched with model `fable` runs solo: `../../../bin/fm-spawn.sh` appends the delegation-tool comma-list as an attached `--disallowedTools=<comma-list>` value so Claude's variadic parser cannot consume the following launch brief, and any dispatch that wants a Fable crewmate delegating further needs the captain's explicit approval for that exact tree (`FM_FABLE_ALLOW_DELEGATION=1`), per `../../../docs/subagent-guard.md`.
