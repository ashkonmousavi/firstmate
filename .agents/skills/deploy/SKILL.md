...
description: >-
  ...
  Everything that can go live without the captain already has; this is only the report.
user-invocable: true
metadata:
  internal: true
---
...
The captain's standing instruction: everything that can be deployed without the captain
is deployed automatically the moment it merges. `/deploy` is how the captain checks, and
the list shown must contain only what the captain actually has to decide.
...
- **Things are waiting only on the captain.** List exactly those, each with the
  concrete reason it needs the captain (the design surface it touches). Then say plainly
  that the captain can approve them and they go live. Nothing else belongs in this list.
...
If the captain approves, deploy exactly what was approved:

```
bin/fm-deploy.sh <project> <sha> --with-captain-permission "<the captain's own words>"
```

Quote the captain's actual words. The flag by itself is not permission, and the deploy
records those words. Give the captain the outcome in one line afterwards.
...
Do not deploy a change that touches a captain-reserved surface without the captain's word
in this session, and never treat a standing `yolo` posture as that word: `yolo`
governs merging, and this gate is about what the captain sees on the live site.
