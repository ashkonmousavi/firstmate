...
- When offering `later`, put `defer_until: "YYYY-MM-DD"` on that option with the date the call should return; the board's payload validator refuses an undated `later` choice.
- Put `defer_until` only on the `later` option; the validator refuses it on any other option, which keeps its card's `done` or `release` close.
...
Decision answers need no routing from you: the runner feeds the board's binding into `bin/fm-captain-hold.sh`'s one keyed-answer intake, which closes, releases, or dates a deferral at answer time; investigate any `skipped:` key against the captured choice and current task before applying the appropriate direct command, and when the captain's answer is "later" but did not arrive as a dated deferral, record it with `bin/fm-captain-hold.sh hold <id> --reason "<existing reason>" --until <date>` instead of a closure.
...
The rollout-compatible old context still feeds ordinary non-reconcile answers, but its bare or separator-annotated reconcile and later values and every structurally uncertain choice feed neither intake and remain announced for deliberate handling.
