...
Firstmate is its own repo, so tracked material (`AGENTS.md`, `bin/`, `.agents/skills/`, and public `skills/`) reaches `main` through the current direct-PR informed-review contract and its real required CI before each running firstmate pulls it.
