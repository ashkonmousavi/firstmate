...
- **Fallback** - if Context7 is unavailable, does not carry the library, or its content is stale, record that limitation explicitly and verify against the primary source instead; never substitute recalled API behavior or a fabricated tool-use claim.
