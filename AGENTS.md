...
config/claude-worker-settings.json  optional JSON object of Claude Code settings merged into every Claude worker launch, e.g. to switch off add-on servers workers never use; LOCAL, gitignored; inherited by secondmate homes; see docs/configuration.md "Claude worker settings"
...
Pass that rule to `fm-spawn` as `--dispatch-rule` (omit it for `default`); spawn refuses a ship or scout harness, model, and effort that match none of that rule's candidate profiles (with no configured `default`, the static crewmate harness passes for `default`) unless `data/<id>/dispatch-override` records an explicit captain override.
...
Run `bin/fm-dispatch-resolve.sh` directly on the written brief in the same turn, with no preflight; on `clear` take its matched rule and resolve that rule's profile as above, while `ambiguous`, `escalate`, `error`, and off all mean the intake above, unchanged (contract: `docs/configuration.md` "Typed dispatch resolution").
...
If `state/.afk-contract` or `state/.afk` is present, load `/afk` in away mode or `/quiet` in quiet mode (`bin/fm-wake-lib.sh`'s `fm_afk_mode`); where its daemon runs, let the daemon own supervision rather than arming another cycle, and on Pi or Codex keep the ordinary supervision session, which runs in both postures (on Pi with main parked while the record exists).
...
**When a navigation-scoped secondmate is registered, consult it for advisory preparation before planning a new work wave, after material scope or shared-contract changes or dependency landings, and whenever sequencing is uncertain; its exact domain responsibility is defined by its `data/secondmates.md` entry and charter.
Primary accountability remains with the firstmate, and independently authorized work does not wait for that advice.**
...
When a step in that spec has a tool that clearly serves it, name that tool for that step rather than leaving the worker to guess or appending a bare list of available tools.
