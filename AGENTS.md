...
Invoke the `/afk` skill when the captain says `/afk`, says they are going afk, `state/.afk` exists, an incoming message is a typed `away-supervisor` input or legacy bare `FM_INJECT_MARK`, or any `state/.subsuper-*` marker is involved.
A typed `watcher` input while away mode is inactive follows the ordinary wake-handling path above and does not invoke `/afk`.
The skill owns the daemon procedure; these safety facts remain inline:

- Every current daemon injection uses the owner-specific kind from `bin/fm-operational-input.sh` after `FM_OPERATIONAL_PREFIX` (U+2063 INVISIBLE SEPARATOR followed by `FIRSTMATE_OP: `): `away-supervisor` in away mode and `watcher` for attended Codex, while the `/afk` skill owns legacy bare-marker compatibility.
