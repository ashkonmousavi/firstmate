...
Invoke the `/afk` skill when the captain says `/afk`, says they are going afk, `state/.afk` exists, an incoming current or legacy operational input classifies as `away-supervisor`, or any `state/.subsuper-*` marker is involved.
The skill owns the daemon procedure; these safety facts remain inline:

- The daemon uses the `watcher` operational kind for attended Codex delivery and `away-supervisor` while `state/.afk` exists; only the latter enters away-mode handling, while the `/afk` skill owns legacy bare-marker compatibility.
