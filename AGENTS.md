...
- `research-first-decisions` - load before selecting a tool, library, framework, service, vendor, or approach from candidates, and before commissioning or consuming research that will make that selection. Before adopting, configuring, upgrading, or integrating a library, SDK, API, CLI, framework, or service, or before debugging version-sensitive usage, every agent - captain-facing and worker alike - must resolve it in Context7, query the documentation needed, match it to the installed or proposed version, and record the library ID, version, date, sources, and decision in the task's own preparation; that skill's Context7 section owns the exact procedure, the receipt fields, the reuse rule, and the primary-source fallback for when Context7 is unavailable, missing the library, or stale. This does not apply to unrelated business-logic edits or every tool invocation.
