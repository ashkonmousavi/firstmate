...
Pass that rule to `fm-spawn` as `--dispatch-rule` (omit it for `default`); spawn refuses a ship or scout harness/model that does not match the selected profile unless `data/<id>/dispatch-override` records an explicit captain override.
