...
# identity-matched daemon holding this home, plus a beacon fresh within the
# poll-derived AFK grace, is what proves supervision there - see
# fm_afk_daemon_owns_supervision and fm_poll_derived_grace in bin/fm-wake-lib.sh.
# The strict watcher predicate and flat grace are unchanged everywhere else.
#
...
# A daemon can legitimately finish handling a slow wake after the flat grace.
# Only this away-owner branch widens freshness to max(300, FM_POLL + 60); a dead
# daemon or a beacon older than that derived bound still blocks.
...
block_stop() {
  ...
  rule='━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
  {
    ...
    elif [ "${FM_SUP_IDLE_CAPACITY:-false}" = true ]; then
      printf '●  Ready work could be dispatched into a free slot right now, but no live watcher holds this home lock (last beat: %s).\n' "$FM_SUP_BEACON_DESC"
    else
...
if [ "$terminal_status" -eq 0 ]; then
  if [ "$FM_SUP_IN_FLIGHT" -gt 0 ]; then
    ...
    NEED_DESC="ready work is dispatchable into a free slot"
  else
