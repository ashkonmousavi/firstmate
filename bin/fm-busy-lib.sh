...
#      muse session-log and cursor transcript pull sources, then the Grok
#      temporary regex fallback classifies a grok task from its
#      rendered tail, then unknown missing
#   5. malformed, stale, or untrusted records -> unknown, never a fallback
# Grok is the only rendered-text classification that survives the redesign,
# because its structured lifecycle was not credited live-verified in the
# approved audit. It is scoped to its own harness= and can never classify
# another adapter. The delivery
