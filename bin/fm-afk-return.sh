...
return_reconcile() {
  ...
  local archived_contract tag kind text retained_live restored_epoch native_watch native_watch_fired
...
EOF
  ...
  if [ -e "$STATE/.afk-native-wedge-watch" ]; then
    ...
    # A recorded wedge proves only that the daemon detected one. This process
    # cannot inspect whether the harness-native watch delivered its alert.
