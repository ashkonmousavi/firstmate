...
# `git push`. No owner marker grants a bypass. A future direct push to
# main/master is refused by the guard and must land through a PR or the
# guarded local merge instead (docs/push-guard.md).
...
require_released_captain_hold() {
  ...
  case "$hold_status" in
    0) echo "error: task $ID is still held for the captain; release it before merging" >&2; return 1 ;;
    1|3) return 0 ;;
    *) echo "error: could not determine whether task $ID is still held for the captain; refusing to merge" >&2; return 1 ;;
  esac
}
...
case "$PROVIDER" in
  github)
    ...
    merge_output=$(gh-axi pr merge "$PR_NUMBER" --repo "$PR_OWNER/$PR_REPO" \
      "${merge_args[@]+"${merge_args[@]}"}" --match-head-commit "$FM_PR_GITHUB_HEAD" "$@" 2>&1) || merge_status=$?
