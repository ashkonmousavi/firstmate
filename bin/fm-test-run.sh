...
family_for_basename() {
  case "$1" in
    ...
    fm-backend-herdr-stale-active-tab-e2e.test.sh|fm-backend-herdr-sidebar-tab-e2e.test.sh|\
...
tests/fm-dispatch-select.test.sh 1800
...
tests/fm-prep-install.test.sh 6000
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-dispatch-select.sh|bin/fm-crew-dispatch-validate.jq)
      ...
      # bin/fm-bootstrap.sh validate against.
      printf '%s\n' session-bootstrap
      printf '%s\n' "__script__:fm-dispatch-select.test.sh"
      ;;
    bin/fm-env-lib.sh)
      # The one .env accessor, sourced by bin/fm-x-lib.sh (Relay token) and
      # bin/fm-dispatch-resolve.sh (TYPESAFE_API_KEY, AI_GATEWAY_API_KEY).
      printf '%s\n' pr-forge
      printf '%s\n' "__script__:fm-dispatch-resolve.test.sh"
      ;;
