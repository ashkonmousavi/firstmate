...
repair_line() {
  ...
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s%s\n' "$prefix" 'persistent supervision is Stop-owned; restore the tracked Codex hooks and retry Stop from Herdr or tmux, without starting another foreground checkpoint.'
      ;;
    pi|pi-signed)
...
ordinary_wake_line() {
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s\n' '- Ordinary wake: the persistent Stop-owned successor already owns watcher continuity; drain and handle the wake, and do not start another foreground checkpoint.'
      ;;
    pi|pi-signed)
