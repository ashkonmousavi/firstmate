...
repair_line() {
  ...
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s%s\n' "$prefix" 'repair missing attended Codex supervision with bin/fm-afk-launch.sh start-codex.'
      ;;
    pi|pi-signed)
...
ordinary_wake_line() {
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s\n' '- Ordinary wake: the attended daemon already owns watcher continuity; drain and handle the wake, and do not start another owner.'
      ;;
    pi|pi-signed)
