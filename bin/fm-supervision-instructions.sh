...
repair_line() {
  ...
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s%s%s%s\n' "$prefix" 'repair missing watcher supervision by letting the tracked Codex Stop hook re-establish the identity-matched persistent successor; only if that hook reports the successor unavailable, use one diagnostic foreground checkpoint: bin/fm-watch-checkpoint.sh --seconds ' "$checkpoint_seconds" '.'
      ;;
    pi|pi-signed)
...
ordinary_wake_line() {
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s\n' '- Ordinary wake: the identity-matched persistent successor already owns watcher continuity; drain and handle the wake, and do not start another foreground checkpoint.'
      ;;
    pi|pi-signed)
