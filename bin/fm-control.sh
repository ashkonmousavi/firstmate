...
resolve_relaunch_profile() {
  ...
  # refuses here first.
  local dispatch_config="${FM_CONFIG_OVERRIDE:-$FM_HOME/config}/crew-dispatch.json" target_effort=$TARGET_EFFORT
  ...
  if [ "$KIND" != secondmate ] && [ -f "$dispatch_config" ] \
    ...
    fm_dispatch_table_admits "$FM_ROOT/bin" "$dispatch_config" "$DATA/$ID/dispatch-override" \
      default "$TARGET_HARNESS" "$account_model" "$target_effort" || return 1
  fi
}
