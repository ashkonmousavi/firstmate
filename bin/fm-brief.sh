...
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations. Before adopting, configuring, upgrading, or integrating a library, SDK, API, CLI, framework, or service, or before debugging version-sensitive usage, use Context7 (\`resolve-library-id\` and \`query-docs\`, or their CLI form) to verify the behavior against the installed or proposed version, and record the library ID, version, date, sources, and decision in your prep; see \`$FM_ROOT/.agents/skills/research-first-decisions/SKILL.md\` for the exact procedure and fallback.
