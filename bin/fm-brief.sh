...
# placeholder and sibling "Resource: {RESOURCE}" RAM/disk-envelope,
# "Surface: {SURFACE}" dashboard-wiring, and "Journey: {JOURNEY}" preparation
# placeholders Firstmate fills at intake
# the same way it fills delivery mode and test scope (AGENTS.md section 11).
# bin/fm-spawn.sh refuses a ship spawn whose brief has no "Prep:" line at all,
# and, when the brief carries a Proof bar section and its task text mentions
# test/build/battery/lint/browser execution, a brief with no filled "Resource:"
# line either. It applies the same refusal to a missing or unfilled "Surface:"
# line, and to an unfilled (never a missing) "Journey:" line,
# whenever the Proof bar section is present and the project is not
...
# loaded. Firstmate fills the {PREP}, {RESOURCE}, {SURFACE}, and {JOURNEY}
# placeholders at
# intake, on the same footing as delivery mode and test scope (AGENTS.md
# section 11); bin/fm-spawn.sh refuses a ship spawn whose brief carries no
# "Prep:" line, whose Prep line is the unfilled "Tier 1" placeholder, or whose
# Prep line names no tier. It applies the same refusal to a missing or unfilled
...
# suite, and to a missing or unfilled "Surface:" line - and to an unfilled
# "Journey:" line, absence never counting - whenever the Proof bar
# section is present and the project is not firstmate itself - a legacy brief
# with no Proof bar section keeps spawning unchanged.
...
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations. Select tools only for a real task purpose. For an unresolved version-sensitive library, SDK, API, CLI, framework, or service question, prefer exact installed CLI help and upstream source, use Context7 when its versioned documentation is the effective source, and use Exa (\`mcp exa web search\` or fetch) or ordinary web search only as a bounded route to otherwise unavailable official primary material. Record the question, tool, exact version, useful evidence, decision, and bounded fallback in prep; see \`$FM_ROOT/.agents/skills/research-first-decisions/SKILL.md\` for the exact procedure.
...
# including its conditional integration-batch owner binding. bin/fm-promote.sh
# renders the same block so a promoted scout receives the same contract.
...
When this task changes a shared symbol, contract, or route and consumer tracing has a real purpose, query the project's existing GitNexus main index with the explicit repository name \`fm-<project-name>\`. Never run \`gitnexus analyze\` inside your worktree; the index lives in a firstmate-owned mirror and is refreshed by bin/fm-gitnexus-reindex.sh. The CLI equivalents are \`gitnexus context <symbol> --repo fm-<project-name>\`, \`gitnexus impact\`, \`gitnexus query\`, and \`gitnexus cypher\`. Compare the index commit with the changed source and record both; if the index is absent, stale, or inapplicable to the relevant language, use direct callers, tests, and config reads and state that precise limitation.
