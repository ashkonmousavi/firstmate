...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, the curated `/heal` diagnostic ledger under `data/heal/`, per-project operational facts in `data/ops-facts.md`, backlog, briefs, scout reports, and explicitly installed content-addressed extension packages under `data/extensions/packages/`.
...
It is read through `bin/fm-supervision-lib.sh`'s `fm_concurrency_cap_value` by the idle-capacity report, which never proposes dispatching more work once this home's own live worker count (`state/*.meta`) is at or over the cap, regardless of any project's own free worktree slots, and by the lane floor below, which reports a shortfall at the cap as capacity-blocked rather than as a dispatch it has no way to perform.
...
It is read by the lane-floor report (`bin/fm-supervision-lib.sh`'s `fm_lane_floor_compute`), which compares this home's PRODUCTIVE lanes against the work `fm_dispatchable_work` can enumerate.
That library's own header owns which lanes count as productive, how the rest are accounted for, and what the report prints; [`.agents/skills/lane-floor/SKILL.md`](../.agents/skills/lane-floor/SKILL.md) owns the line format and what to do about it.
A shortfall - fewer productive lanes than the floor while that enumeration is non-empty - is escalated on every wake drain while it holds, and blocks a Claude turn end through [`bin/fm-turnend-guard.sh`](../bin/fm-turnend-guard.sh) (see [turnend-guard.md](turnend-guard.md)).

The cap and the floor bound opposite ends of the same number: the cap is a ceiling on concurrent workers, the floor a minimum on productive lanes while work exists.
The cap wins where they meet: a home already at its cap cannot spawn another worker, so its shortfall is reported as capacity-blocked, naming the lanes it could release instead of demanding a dispatch it has no way to perform.
Every worker counts against the cap, waiting and finished ones included, because each still holds its worktree slot even when it is not a productive lane.
That makes a floor set above the cap harmless rather than unsatisfiable, but it also means such a floor can only ever be met by releasing a lane above the cap.
  ...
  "rules": [
    {
      ...
      "use": [
        { "harness": "<adapter>", "model": "<optional model>", "effort": "<medium|high|xhigh|max, optional>" }
      ],
...
The dispatch table's effort floor is medium, so `low` is invalid.
If a selected profile carries an effort value the chosen harness does not accept, `fm-spawn.sh` refuses before metadata is written, and bootstrap reports the invalid harness/effort pair as a `CREW_DISPATCH` diagnostic when it is visible in the file.
