...
An absent or too-old `quota-axi` reports `MISSING: quota-axi (install: npm install -g quota-axi)`; explicit `quota-balanced` selection cannot resolve without a compatible binary, while ordered resolution never reads it.
...
FM_CREW_STATE_NM_TIMEOUT=10   # seconds allowed per no-mistakes query inside fm-crew-state.sh, and for the watcher gate-nudge ladder's own axi status read
