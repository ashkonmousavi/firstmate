...
When away-mode injection wedges past `FM_MAX_DEFER_SECS`, the sub-supervisor raises an alarm, clears its legacy `state/.afk` flag, and exits its tracked background job so ordinary turn-end supervision can resume.
...
A missing or failing channel logs and falls through to the next before the daemon exits on the undeliverable escalation.
...
The watcher counts every live ship task as an occupied lane, whether it is working, parked at a gate, or paused, and reads `fm-tasks-axi.sh ready` for dependency, date, and hold eligibility.
...
`bin/fm-dispatch-select.sh` is the stateless selector for an already-selected rule index or `default`; its header and `--help` own arguments, availability-fact format, output, and exit codes.
...
Malformed JSON, an empty or malformed rule/default array, an unknown selection mode, a non-boolean `off`, an unverified harness, or an effort value unsupported by that harness is reported as `CREW_DISPATCH: invalid config/crew-dispatch.json - ...`.
...
On `clear`, firstmate takes the tool's answer as the matched rule, where `rule_<n>` is rules index `n-1`, a `fallback:` line names the rule actually resolved, and a below-floor note means `default`.
...
FM_MAX_DEFER_SECS=300              # max buffered escalation age before retry, alarm, and daemon handback; 0 disables
