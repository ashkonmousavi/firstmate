...
Claude's Stop `asyncRewake` hook owns tokenless re-arm cycles, Cursor's stop hook parks on the watcher, Grok uses background-notify cycles, Codex hands one bootstrap checkpoint to a persistent Stop-owned successor, Pi and pi-signed use the same two tracked primary extensions, and OpenCode uses its TUI plugin.
...
FM_CODEX_WATCH_CHECKPOINT=180   # seconds for Codex's one bootstrap checkpoint before persistent Stop handoff
