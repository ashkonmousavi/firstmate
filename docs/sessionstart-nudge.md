...
Interactive compaction and re-emit remain unverified; startup delivery does not establish those reset paths.
...
| Codex exec | Run | `.codex/hooks.json` anchors to the hook process working directory, verifies a Firstmate-shaped hook-bearing root, and pipes the hook payload into the wrapper with `--harness codex` and a 180s timeout. | Native stdout context injection is supported under `codex exec`. |
