...
| --- | --- | --- |
| Run | Executes `bin/fm-session-start.sh` through the native session-open adapter and gates its ordered digest into model context before the first turn. | Claude, Codex startup, Pi / pi-signed, omp, Cursor |
...
Codex 0.154.0's interactive TUI delivers the tracked project `SessionStart` hook at startup when project hooks are trusted.
Interactive compaction and re-emit remain unverified.
...
| Codex interactive TUI startup | Run on 0.154.0 | [Codex interactive TUI](#codex-interactive-tui) |
...
The trusted Codex hook carries the same command with `--codex-hook` so the digest uses that hook's verified identity rather than process ancestry.
...
Codex 0.154.0 fires the trusted tracked project `SessionStart` hook in its interactive TUI and injects the startup digest before the first turn.
The earlier 0.146.0 negative observation is superseded for startup.
Interactive compaction and re-emit remain unverified, and Firstmate ships no global hook for them.
