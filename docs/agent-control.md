...
1. **Resolve the profile.**
   ...
   When `config/crew-dispatch.json` is active, a ship or scout relaunch that changes harness, model, or effort must pass the same [crew-dispatch table check](configuration.md#crew-dispatch-profiles-configcrew-dispatchjson) a fresh spawn does, and a refused profile also refuses before the old agent stops.
2. **Safe checkpoint.**
