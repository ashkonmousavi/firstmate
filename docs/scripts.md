...
| `fm-brief.sh`            | Scaffold the tiered task preparation record (`--prep`), and ship (explicit `--mode`), scout, secondmate-charter, and Herdr-lab briefs, with Captain's intent and Firstmate spec subsections on ship/scout |
| `fm-prep-install.sh`     | Install a filled secondmate `nav-preps/<id>.md` into this home's `data/<id>/prep.md` |
| [`fm-dod-lib.sh`](../bin/fm-dod-lib.sh) | Own ship/scout worker role scope, ship definitions of done, the no-mistakes `--intent` contract, and the task preparation record's tiers, sections, and validation |
