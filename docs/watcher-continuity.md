...
An unacknowledged downtime generation is announced at most once per down stretch: the first recovery marks that generation announced, and later arms wait until a watcher close or durable queue append republishes downtime.
A watcher start alone is not a down stretch, because Claude's Stop hook arms every cycle as a non-successor start.
A non-successor start therefore mints a fresh generation for an announced-but-unacked episode only while its durable queue rows remain unacknowledged, so an idle goal-only or no-wake turn never re-announces a recovery that has no new work.
Every watcher close except the one that delivered the recovery wake, and every durable queue append, publishes downtime.
A downtime republication of any pending or announced episode reuses its generation instead of minting a new one and returns it to pending, so that down stretch is presented once more and buried decisions still resurface.
...
It also drives Claude-style bare Stop re-arms across goal-only turns: a delivered recovery is never repeated when there is no new work, a genuine down stretch whose handshake is lost is announced exactly once, and queued work, a pending inbox record, an open decision, and crew close work still surface.
