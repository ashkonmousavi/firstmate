...
Codex uses one bounded bootstrap checkpoint, then the Stop hook verifies one identity-matched persistent daemon/watcher owner before the captain-facing turn can finish.
...
Claude depends on the Stop `asyncRewake` rewake, Cursor depends on its awaited stop-hook park, Grok retains native background-completion notifications, and Codex depends on its persistent Stop-owned successor after one bootstrap checkpoint.
