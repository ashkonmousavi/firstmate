...
Ordinary `bin/fm-pr-check.sh` registration records `pr_head=` only for GitHub, while its documented `--absorbed-by` path requires GitLab JSON support and records the combined head needed for exact constituent containment.
