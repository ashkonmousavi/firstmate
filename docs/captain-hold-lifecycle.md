...
The optional mode column carries the selected choice's close: `done` (default) completes the task, `release` lifts the hold so held work resumes, and `defer:YYYY-MM-DD` re-holds an open captain call until that future date, reported as `deferred:` with no resolution record; any other value is skipped.
