...
It also drops bare and separator-annotated legacy `later` values, which carry no dated deferral.
Each dropped legacy row still replaces an earlier answer to the same card, leaving that call held.
...
- A `later` option requires a valid `defer_until` date that no other option may carry.
