...
Immediately after the primary-scope check, and before evaluating supervision need at all, the guard checks lock ownership with `fm_session_lock_owned_by_self` (`bin/fm-session-lock-lib.sh`).
When this session does not own `state/.lock` and the recorded pid is a verifiably live harness process (`fm_harness_pid_alive`), a different live session owns this home's primary session right now.
This session's own repair authority - the Stop-owned auto-arm and watcher arming - is keyed to owning that lock exactly the same way (`bin/fm-claude-stop-autoarm.sh` stands down identically), so it has no way to fix supervision here even if it blocks.
...
`tests/fm-turnend-guard.test.sh` covers the predicate, main and secondmate primary scope, child-worktree exclusion, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, the live-lock and fresh-beacon guard predicate, the cooperative `--claude` open-generation claim wait, monotonic failed-epoch progression, bounded attended fail-open, post-alarm continuation suppression, positive recovery reset, generation and legacy claim cases that must block or clear instead of allowing a blind stop, away-mode daemon ownership between watcher cycles and over a watcher lock left behind by an exited watcher, plus its dead, pid-reused, absent, stale-beacon, and away-mode-off negatives, Pi logical-run latching, missing-`jq` behavior, all five primary registrations, Grok native and legacy selection, typed field precedence, malformed input, exactly-one-path safety, and the foreign-lock-owner defer in both default/Codex and `--claude` modes plus its dead-owner negative control.
`tests/fm-sessionstart-advisor-role.test.sh` covers the `FM_SESSION_ROLE=advisor` session-open marker directly and under both the tracked Claude and Codex `SessionStart` hook entry points, an ordinary primary session's unaffected digest-reaching behavior, and a crashed-previous-owner lock left untouched by an advisor launch.
