...
An undeliverable max-defer escalation or a watcher that cannot start sends the active alert, requeues the escalation buffer as a durable wake row, and exits the tracked daemon so the ordinary turn-end path delivers it.
...
Codex remains inside its bounded foreground checkpoint loop, which returns durable watcher output as a tool result without modifying the composer.
Another opted-in non-Pi home also skips the daemon for `/afk`; see [supervision-host.md](supervision-host.md).
