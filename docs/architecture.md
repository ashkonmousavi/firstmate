...
Its budget is bound to a gate identity made of the gate detail, the task's status-log signature, the task worktree head, and the id and head of the run itself as a bounded `no-mistakes axi status` in the task worktree reports them, so a gate after the worker's own commit earns its own rings.
A no-mistakes fix round commits in the pipeline's own checkout instead, so the task head never moves and the next gate can report the same step and finding count, and the worker may stay busy through the whole round, so no probe ever sees the run working in between; the run head that fix round advances is what separates those two gates, since `axi status` exposes no per-gate round while a run is parked.
