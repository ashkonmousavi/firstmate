...
Its budget is bound to a gate identity made of the gate detail, the task's status-log signature, and the task worktree head, so a gate after the worker's own commit earns its own rings.
A no-mistakes fix round commits in the pipeline's own checkout instead, so the task head never moves and the next gate can report the same step and finding count; what separates those two gates is the run itself reading as working in between, so a working line from the run's own `axi status` resets the ladder, spent budget included.
Every other read that finds no gate keeps the identity and ring count, so a probe that timed out or a coarse runs-list read that cannot see the gate can neither restart the ladder mid-way nor buy a second pair of rings.
...
`tests/fm-watch-gate-nudge.test.sh` pins the first ring, the second ring, escalation only after two, a budget that survives a transient non-gate read, fresh rings for a same-looking gate after the run resumes, a green-CI ring after an earlier round's `done:`, the spent budget that neither rings nor wakes firstmate again, the busy-pane, open-decision, declared-wait, scout, secondmate, already-reported and dead-endpoint exclusions, and the re-arm on a new worktree head.
