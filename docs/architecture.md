...
A spent record keeps that identity even across a read that finds no gate at all, so a probe that timed out or a run that flickers back to running between two reads of one gate cannot buy a second pair of rings.
...
`tests/fm-watch-gate-nudge.test.sh` pins the first ring, the second ring, escalation only after two, the spent budget that neither rings nor escalates again, the busy-pane, open-decision, declared-wait, secondmate, already-reported and dead-endpoint exclusions, and the re-arm on a new worktree head.
