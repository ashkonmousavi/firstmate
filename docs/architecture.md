...
Before any of that escalation runs, an idle ship crew pane whose no-mistakes run has parked at an approval or fix-review gate, or whose PR checks are green while only the worker's own `done:` report is outstanding, is rung instead of escalated.
Only a latest `done:` line that names the PR and `checks green` counts as that report, the predicate `bin/fm-crew-state.sh` uses, because the brief has the worker append a `done:` summary before validation starts and that line is usually still the last one when CI goes green.
no-mistakes pushes nothing when a gate opens, and the wedge timer by contract never re-reads crew state, so the previous behavior was that a worker which ended its turn while its run was still `running` sat at an empty prompt until the wedge escalation `FM_STALE_ESCALATE_SECS` later, at which point firstmate rang it by hand.
The nudge ladder performs that ring itself: it writes one fire-and-forget steering-inbox record naming the gate, the step and finding count the current-state line reports, and the exact next command, then rings the ordinary doorbell.
Fire-and-forget is what keeps the prompt out of the steering re-ring ladder, since it prompts the worker to act on state it can already read rather than carrying an instruction whose loss firstmate must hear about.
A second ring follows only if the pane stays idle for another `FM_GATE_NUDGE_SECS` with the same gate still open, and after two unanswered rings the ladder emits the ordinary stale wake itself, carrying a `gate-nudged x2` marker so firstmate reads it as an ignored ring rather than a fresh wedge.
The ladder emits that wake rather than handing the pane back, because the pane it escalates is usually one the wedge timer already absorbed, and handing it back would restore the very delay the ladder removes.
That escalation also spends the budget: firstmate is woken about one gate once and owns the pane from there, so the ladder hands every later poll back to the unchanged triage rather than re-escalating on its own cadence.
It also marks the pane's current hash as already surfaced, exactly as the ordinary stale surfacing does, so the unchanged triage only starts its wedge timer on that hash instead of waking firstmate a second time about the same gate.
Its budget is bound to a gate identity made of the run's own id, the gate step, and the run's head, as a bounded `no-mistakes axi status` in the task worktree reports them; the step is the one the current-state line names, or `ci` for a green-checks gate.
A no-mistakes fix round advances the run head in the pipeline's own checkout, and the worker may stay busy through the whole round, so no probe ever sees the run working in between; the run head is what separates those two gates at the same step, since `axi status` exposes no per-gate round while a run is parked.
A status line the worker appends, or a commit it makes in its own task worktree, while a gate stays open is not a new gate and leaves the budget as it is.
A working line from the run's own `axi status` also resets the ladder, spent budget included.
An `axi status` read that fails, times out, or answers for another branch's run cannot tell one gate from the next, so it rings nobody on that probe and hands the pane back to the unchanged triage with the budget as it was.
Every other read that finds no gate keeps the identity and ring count too, so a probe that timed out or a coarse runs-list read that cannot see the gate can neither restart the ladder mid-way nor buy a second pair of rings.
Such a read only hands the pane back to the unchanged triage until a later probe finds the gate open again.
Both clocks are deliberate: the record's own epoch in `state/.gate-nudge-<task>` paces one bounded current-state probe per `FM_GATE_NUDGE_SECS` per pane, because the pane-idle clock is reset by the doorbell this ladder types, while the pane-idle clock is what decides whether the worker responded to a ring.
Between those paced probes the ladder also probes a pane that has settled on a newly distinct stale hash, one the ordinary triage has not classified yet, because that is where a gate re-parks after the worker answered the last one, and handing that hash to the triage would wake firstmate about the new gate before the worker is rung.
That is also the cost bound: a probe is one crew-state read, once per `FM_GATE_NUDGE_SECS` per pane and otherwise only on a newly distinct stale hash, the same hashes on which the ordinary triage already pays its own crew-state read, and the pacing record plus the triage's classified-hash record are what keep it from repeating every poll.
A scout is outside the ladder because `bin/fm-crew-state.sh` reads a no-mistakes run only for a ship crew, so a scout never shows a gate, a secondmate is outside it because its idle endpoint is healthy by design, an away posture is outside it because the daemon owns triage, a task that already carries an open decision is outside it because firstmate, not the worker, answers next, a declared external wait or captain hold is outside it because the bounded pause cadence owns that pane and deliberately costs no crew-state read, and a positively dead or missing endpoint is outside it because there is nobody to ring and the ordinary path already routes that to recovery.
`tests/fm-watch-gate-nudge.test.sh` pins the first ring, the second ring, escalation only after two, a budget that survives a transient non-gate read, fresh rings for a same-looking gate after the run resumes or its head moves while the worker stays busy, a ring rather than a bare stale wake for a gate that re-parks onto a new pane hash inside the nudge interval, a green-CI ring after an earlier round's `done:` or a pre-validation `done:` summary, the spent budget that neither rings nor wakes firstmate again, the busy-pane, open-decision, declared-wait, scout, secondmate, already-reported, dead-endpoint and unreadable-run exclusions, the re-arm of a spent budget on a new run head but not on an unchanged one, and a budget that a status append or a local commit does not reset.
...
On Pi, pi-signed, and Codex the away daemon is no longer launched: the ordinary supervision session continues under the record.
Codex keeps its bounded foreground checkpoint loop, including during away posture; [`supervision-protocols/codex.md`](supervision-protocols/codex.md) owns its recovery and guarded legacy-handoff procedure.
...
`docs/configuration.md` owns selection semantics; `bin/fm-dispatch-resolve.sh` resolves the chosen rule from supplied facts or hands explicit quota-balanced selection to `quota-array-dispatch`.
