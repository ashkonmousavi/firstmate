...
That pacing record is also the cost bound: on a pane with no gate the probe is one crew-state read in addition to the one the ordinary triage makes on each newly distinct stale hash, and the record is what keeps it from repeating every poll.
