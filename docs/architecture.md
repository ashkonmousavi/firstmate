...
Before any of that escalation runs, an idle ordinary crew pane whose no-mistakes run has parked at an approval or fix-review gate, or whose PR checks are green while only the worker's own `done:` report is outstanding, is rung instead of escalated.
