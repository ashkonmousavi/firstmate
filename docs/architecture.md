...
Its delivery preflight binds the clean-worktree check, the unresolved-placeholder scan, and every permitted delivery action to the worktree named by the task record and the exact `fm/<id>` delivery ref, so a nested or unrelated repository cannot satisfy the gate for the actual task.
