...
Its budget is bound to a gate identity made of the gate detail, the task's status-log signature, and the task worktree head, so a later fix round that reports the same step and finding count still earns its own rings, and only such a new identity re-arms a spent budget.
The record keeps that identity and its ring count even across a read that finds no gate at all, so a probe that timed out or a run that flickers back to running between two reads of one gate can neither restart the ladder mid-way nor buy a second pair of rings.
...
`tests/fm-watch-gate-nudge.test.sh` pins the first ring, the second ring, escalation only after two, a budget that survives a transient non-gate read, a green-CI ring after an earlier round's `done:`, the spent budget that neither rings nor escalates again, the busy-pane, open-decision, declared-wait, scout, secondmate, already-reported and dead-endpoint exclusions, and the re-arm on a new worktree head.
