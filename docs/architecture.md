...
Its budget is bound to a gate identity made of the gate detail, the task's status-log signature, and the task worktree head, so a later fix round that reports the same step and finding count still earns its own rings.
...
A secondmate is outside the ladder because its idle endpoint is healthy by design, an away posture is outside it because the daemon owns triage, a task that already carries an open keyed decision is outside it because firstmate, not the worker, answers next, and a positively dead or missing endpoint is outside it because there is nobody to ring and the ordinary path already routes that to recovery.
`tests/fm-watch-gate-nudge.test.sh` pins the first ring, the second ring, escalation only after two, the busy-pane, open-decision, secondmate, already-reported and dead-endpoint exclusions, and the re-arm on a new worktree head.
