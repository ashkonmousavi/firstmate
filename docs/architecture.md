...
`bin/fm-dod-lib.sh` is the one owner of the shared batched-findings review rule and each mode's definition of done, rendered both into a generated ship brief and into the ship instructions a promoted scout receives, so a promoted worker cannot be handed a weaker contract than a briefed one.
It is also the one owner of the no-mistakes `--intent` contract those workers follow.
Its delivery preflight binds the clean-worktree check, the unresolved-placeholder scan, and every permitted delivery action to the worktree named by the task record and the exact `fm/<id>` delivery ref, requires every recursively referenced submodule to be inspectable at its recorded commit, and scans those commits too, so a nested, unrelated, or incomplete repository cannot satisfy the gate for the actual task.
...
When current main moves or compatible ready work may share one combined delivery, [the `integration-batch-delivery` skill](../.agents/skills/integration-batch-delivery/SKILL.md) owns the choice and lifecycle rather than this architecture overview.
...
For an absorbed constituent, `fm-pr-check.sh --absorbed-by` preserves its original pull request, branch, exact head, and superseded disposition before replacing canonical `pr=` with the combined landing; teardown additionally requires that recorded head inside the combined pull request's own permanent `refs/pull/<n>/head`, and the commit the forge reports that merge produced present on the default branch.
The constituent head itself is never required on the default branch, because a project may land every commit there as a squash merge.
...
The primary's tracked bytes are installed in place, but its already-running conversation keeps its frozen instructions, loaded skills, hooks, and launch-time wiring; reading the changed files supports a safe handoff only, and the installed behavior becomes effective in a fresh primary session.
