...
It proves a TypeSafe success makes no gateway call, while a TypeSafe HTTP or transport failure with `AI_GATEWAY_API_KEY` set (environment winning over `.env`) retries once through the gateway endpoint with the same Choice body under the `typesafe-ai/jev` model, names the answering provider, keeps both keys off argv, output, and child environments, and reports a double failure as one `error` carrying each provider's HTTP outcome and response excerpt.
